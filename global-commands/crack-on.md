As your very first action, start the session clock: run `mkdir -p .tmp && for f in .tmp/.session-start-*; do pid="${f##*-}"; kill -0 "$pid" 2>/dev/null || rm -f "$f"; done && date -u +%Y-%m-%dT%H:%M:%S+00:00 > .tmp/.session-start-$$` — this timestamps when the session began.
